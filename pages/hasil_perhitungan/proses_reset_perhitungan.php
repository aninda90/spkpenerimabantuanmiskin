<?php
	...
	$jmlOpt = mysqli_query($koneksi_db, "SELECT * FROM hasil_optimasi");
	$jmlPref = mysqli_query($koneksi_db, "SELECT * FROM hasil_preferensi");

	$row1 = mysqli_num_rows($jmlNorm);
	$row2 = mysqli_num_rows($jmlOpt);
	$row4 = mysqli_num_rows($jmlPref);
	...
	if ($row1 == 0 && $row2 == 0) {
		...
		$tabOpt = mysqli_query($koneksi_db, "TRUNCATE TABLE hasil_optimasi");
